<?php
	...
	$wherelist[]=" c.pid=p.id ";
	$wherelist[]=" c.uid=u.id ";
	$wherelist[]=" u.id={$_SESSION['users']['id']} ";
	...
	$sql="select c.id,u.username,p.cid,c.addtime,c.content,p.pname,p.picname
		...
		<link rel="stylesheet" href="public/css/footer.css"/>
		 <script type="text/javascript">
            ...
            function doDel(id){
                if(confirm("确定要删除吗？")){
                    //跳转网页,并携带参数
                    window.location='doaction.php?operate=delcomment&id='+id;
                }
            }
 </script>
	</head>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php 
				...
				$sql="select c.id,u.username,p.cid,c.addtime,c.content,p.pname,p.picname
	from comments c,product p,users u {$where}  {$limit}";
				//echo $sql;die;
				//echo $pagenum;die;
				$res=mysql_query($sql);
				//var_dump($res);die;
				...
				if($res){
						...
						<a title='修改评论' style=\"display:block;\" href='updatecoment.php?id={$rows['id']}' class='ml-5' style='text-decoration:none'>
									 
								<i class='Hui-iconfont'>修改评论</i>
								</a> 
								";
								
						$del="javascript:doDel({$rows['id']});";
								echo "<a href='{$del}' class='ml-5' style='text-decoration:none'>
								<i class='Hui-iconfont'>删除评论</i></a>";
						...
						echo "<a style='padding-left:10px;' href='commentlist.php?p=1{$search}'>首页</a>";
						echo "<a style='padding-left:10px;' href='commentlist.php?p=".($currentpage-1)."{$search}'>上一页</a>";
						echo "<a style='padding-left:10px;' href='commentlist.php?p=".($currentpage+1)."{$search}'>下一页</a>";
						echo "<a style='padding-left:10px;' href='commentlist.php?p={$pagenum}{$search}'>尾页<a/>";
						echo "</td></tr>";
