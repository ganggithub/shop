<?php
	...
	$urlist=array();
	if(!(empty($_GET['pname']))){
		$wherelist[] =" p.pname like '%{$_GET['pname']}%'";
	...
	$wherelist[]=" p.id=d.pid ";
	$wherelist[]=" d.orderid=o.id ";
	$wherelist[]=" o.uid={$uuid} ";
	$wherelist[]=" o.status=5 ";
	//$wherelist[]=" p.isdel=1 ";
	if(count($wherelist)>0){
		$where=" where ".implode(" and ",$wherelist);
		$search="&".implode("&",$urlist);
	}
	/*$sql2="select p.id,p.pname,p.picname,o.total,d.buynum,o.addtime 
	from product p,orders o,orderdetail d
where p.id=d.pid and d.orderid=o.id and o.uid=22 and o.status=5;";*/
	$sql="select p.id,p.cid,p.pname,p.picname,o.total,d.buynum,o.addtime 
	from product p,orders o,orderdetail d
	{$where}";
		...
		<link rel="stylesheet" href="public/css/footer.css"/>
		 <script type="text/javascript">
            ...
            function doDel(id){
                if(confirm("确定要删除吗？")){
                    //跳转网页,并携带参数
                    window.location='doaction.php?operate=delorderdetail&id='+id;
                }
            }
 </script>
	</head>
		...
		<div class="orderbody clear">
			<div class="center">
				...
				<div class="orderright">
						...
						<div class="pd-20">
	<form action="ygmprod.php" method='get'>
	<div class="text-c">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php 
				...
				$sql="select p.id,p.cid,p.pname,p.picname,o.total,d.buynum,o.addtime 
					from product p,orders o,orderdetail d {$where}  {$limit}";
				//echo $sql;die;
				//echo $pagenum;die;
				$res=mysql_query($sql);
				//var_dump($res);die;
				$count=@mysql_num_rows($res);
						if($count==0){
							echo "<tr><td colspan='8'>没有数据,请添加商品吧</td></tr>";
						}
				if($res){
						while($rows=mysql_fetch_assoc($res)){
							...
							echo "<tr class='text-c'>";
								...
								echo "<td>{$rows['id']}</td>";
								echo "<td><a href='prodetail.php?id={$rows['id']}'>{$rows['pname']}</a></td>";
								$cid=$rows['cid'];
						...
						<a title='添加评论' style=\"display:block;\" href='addcoment.php?id={$rows['id']}' class='ml-5' style='text-decoration:none'>
									 
								<i class='Hui-iconfont'>添加评论</i>
								</a> 
								";
								
						$del="javascript:doDel({$rows['id']});";
								echo "<a href='{$del}' class='ml-5' style='text-decoration:none'>
								<i class='Hui-iconfont'>删除订单</i></a>";
						...
						echo "<a style='padding-left:10px;' href='ygmprod.php?p=1{$search}'>首页</a>";
						echo "<a style='padding-left:10px;' href='ygmprod.php?p=".($currentpage-1)."{$search}'>上一页</a>";
						echo "<a style='padding-left:10px;' href='ygmprod.php?p=".($currentpage+1)."{$search}'>下一页</a>";
						echo "<a style='padding-left:10px;' href='ygmprod.php?p={$pagenum}{$search}'>尾页<a/>";
						echo "</td></tr>";
